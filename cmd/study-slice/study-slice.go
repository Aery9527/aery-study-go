...
func main() {
	...
	s6[0] = 9
	s7[1] = 8
	where.WrapPrint("append(slice) <= cap", func() {
		fmt.Printf("s5:%v type(%T) len(%d) cap(%d)\n", s5, s5, len(s5), cap(s5))
	...
	where.WrapPrint("slice[i:j:k]", func() {
	...
	where.WrapPrint("append(slice[i:j:k]) <= cap", func() {
		fmt.Printf("arr:%v type(%T) len(%d) cap(%d)\n", arr, arr, len(arr), cap(arr))
		fmt.Printf("sss:%v type(%T) len(%d) cap(%d)\n", sss, sss, len(sss), cap(sss))
	})
	...
	s0 := s12 // reference
	where.WrapPrint("slice 傳遞是指標, 因此會指向同個底層 array", func() {
