...
func ShowContext() {
	wrap := func(term string, action func()) {
		fmt.Println("[" + term + "]")
		action()
	}
	...
	wrap("withValue", withValue)
	wrap("withTimeout", withTimeout)
	wrap("withoutCancel", withoutCancel)

	wrap("withCancel", func() { withCancel(context.WithCancel(parentCtx)) })
	wrap("WithCancelCause", func() { // 跟 WithCancel 差別在於可以附 cancel cause
	...
	wrap("withDeadline", func() {
		ctx, cancel := context.WithDeadline(parentCtx, duration())
		withDeadline(ctx, cancel, delay)
	})
	wrap("withDeadlineCause", func() { // 跟 WithCancel 差別在於可以附 cancel cause
